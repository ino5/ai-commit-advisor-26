# AI Commit Advisor 시연 사용 가이드

이 문서는 AI Commit Advisor를 처음 보는 팀원이나 이해관계자에게 제품 흐름을 보여줄 때 사용하는 사용자용 가이드입니다. 설치 세부값이나 내부 검증 절차보다, 샘플 프로젝트로 어떤 화면을 어떤 순서로 보고 어떤 질문에 답하는지에 집중합니다.

샘플 프로젝트는 실제 고객 코드 없이도 개발계획, Git 변경 이력, AI 매핑, 리스크, 소스 기반 질의응답, 코드 리뷰 흐름을 한 번에 보여주기 위해 준비된 데모 데이터입니다.

## 시연 목표

이 시연은 다음 질문에 답하는 것을 목표로 합니다.
...
로컬에서 시연할 때는 현재 PC가 앱 서버입니다. 프로젝트/Git 설정에 입력하는 Git 저장소 경로도 현재 PC에서 접근 가능한 경로를 사용합니다.
...
| 설명 | `AI Commit Advisor 시연용 샘플 프로젝트` |
...
- 로컬 시연에서는 내 PC의 샘플 프로젝트 경로가 앱 서버 Git 저장소 경로입니다.
- 사내 서버 시연에서는 서버에 clone된 저장소 경로를 등록해야 합니다.
...
Home은 시연 중 현재 프로젝트의 관제 화면으로 사용합니다. 데이터 수집이나 분석을 실행한 뒤 다시 돌아와 상태가 어떻게 바뀌는지 보여주기 좋습니다.
...
LLM 설정이 `mock`이면 실제 판단 품질보다 화면 흐름 확인에 가깝습니다. 실제 시연 품질을 보여주려면 local LLM 설정을 사용합니다.
...
2. `현재 소스 다시 인덱싱` 또는 `최근 Git sync 변경 파일만 인덱싱`을 실행합니다.
3. Embedding 연결 테스트를 실행합니다.
4. source_file chunk embedding을 작은 수량부터 생성합니다.
...
- AI Code Review는 현재 작업트리, staged 변경, 최신 commit, 특정 commit을 대상으로 실행할 수 있습니다.
- 시연에서는 특정 commit을 선택하면 결과를 설명하기 쉽습니다.
...
시간이 짧으면 RAG Search, Project Chat, AI Code Review는 대표 질문과 대표 commit 1개만 보여줍니다.
