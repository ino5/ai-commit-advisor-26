...
## 2026-06-17 - 샘플 Project Chat 검증에서 source_file 인덱스와 local LLM context 한계를 놓쳤다

분류:

- Sample data verification failure
...
샘플 프로젝트를 다시 생성하고 프로젝트 97을 full Git Sync한 뒤 Project Chat 질문을 테스트했을 때, 첫 실행은 `현재 검증된 소스 근거만으로는 답변하기 어렵습니다.`로 끝났습니다. 이후 release rehearsal, go/no-go, operator handoff 문서명을 포함한 질문은 올바른 source_file을 검색했지만, `TOP K=8`, graph evidence, history context가 함께 들어가면서 local LLM이 HTTP 400 context overflow를 반환해 빈 답변이 저장됐습니다.

### 직접 원인

첫 검증 스크립트가 `build_project_chunks` 기본값만 사용했습니다. 이 함수의 `include_source_files` 기본값은 `False`라서 `commit`, `commit_file`, `program` chunk만 생겼고, Project Chat이 요구하는 verified `source_file` chunk가 없었습니다. 두 번째 문제는 LM Studio에 올라간 `qwen2.5-coder-7b-instruct`가 4096 token context로 동작하는데, release evidence 질문에 graph/history 근거까지 붙이면서 prompt가 context limit을 넘은 것입니다.

### 배경 또는 구조적 원인

Project Chat은 현재 코드 사실을 답할 때 verified `source_file`만 신뢰하도록 설계되어 있습니다. 이 안전 정책은 맞지만, 수동 검증 스크립트가 RAG 화면의 `전체 소스 다시 읽기` 흐름과 같은 `refresh_source_file_index` 단계를 명시적으로 따라가지 않았습니다. 또한 sample demo 검증은 "근거가 많을수록 좋다"는 방향으로 흐르기 쉬운데, 작은 local LLM context에서는 근거 수가 많을수록 실패 가능성이 커집니다.

### 사전 검증에서 놓친 이유

이전 검증은 이미 source_file index가 존재하는 프로젝트 상태에서 진행돼 `source_file` 누락을 드러내지 못했습니다. 또한 UI 기본 `TOP K=8`과 graph evidence가 결합된 Project Chat prompt 길이를 local LLM context 기준으로 별도 점검하지 않았습니다.

### 수정 내용

- 샘플 release evidence에 한국어 판단 요약을 추가해 `준비 완료`, `주의`, `출시 불가`, `운영 인수인계`, `결제 대시보드 정합성 점검` 같은 한국어 질문이 source_file 근거에 직접 매칭되도록 했습니다.
- 프로젝트 97을 reset한 뒤 `refresh_source_file_index`, `build_project_chunks`, local embedding, Mapping, Risk Analysis, Neo4j sync, Project Chat, AI Code Review, PL Briefing을 처음부터 다시 실행했습니다.
- Project Chat 검증에서는 4096 context local LLM 한계를 고려해 source_file 중심 질문과 낮은 `top_k`를 사용하고, UI에는 실제 `answer_source_question` 결과를 저장한 session을 표시했습니다.
- 사용 검증 문서에 `source_file` 88개, 전체 chunk/vector 291개, Mapping 80건, Risk 21건, Neo4j 192 nodes/558 edges, 실제 local LLM review/chat/PL 결과를 기록했습니다.

### 재발 방지 규칙

- 샘플 프로젝트를 처음부터 다시 검증할 때는 Git Sync 뒤 반드시 `refresh_source_file_index`를 실행하고, `source_file` chunk/vector 수를 확인합니다.
- Project Chat 실제 검증은 `source_file` 근거 수, `fallback=False`, `insufficient_evidence=False`, provider/model, context overflow 여부를 함께 기록합니다.
- 4096 token local LLM 환경에서는 release/operations demo 질문의 `TOP K`를 낮추거나 history/graph context를 분리해 검증합니다.
- 스크린샷 증거는 빈 화면이나 pre-execution form이 아니라 provider/model/fallback, source evidence count, graph freshness, review result가 보이는 상태로 남깁니다.

### 남은 한계와 후속 확인

- 현재 Project Chat prompt는 source 근거 원문을 길게 인용하는 경향이 있어, 한국어 질문에도 영어 원문 일부가 함께 출력될 수 있습니다. 샘플 문서에는 한국어 요약을 보강했지만, 답변 스타일 자체를 더 자연스럽게 하려면 prompt 개선이 별도 필요합니다.
- UI 기본 `TOP K=8`은 큰 context 모델에서는 유리하지만 4096 context local LLM에서는 불안정할 수 있습니다. UI에서 모델 context에 따라 권장 `TOP K`를 안내하는 개선은 후속 후보입니다.

### 검증 명령과 결과

- `.\.venv\Scripts\python.exe -m pytest tests\test_sample_data_generation.py -q` 결과 14개 통과.
- `.\.venv\Scripts\python.exe scripts\create_sample_target_repo.py --target-path C:\dev\ai-advisor-sample-shop --force` 성공, 48 commits 생성.
- 실제 local 검증 결과: Git Sync 48 commits/110 files, source_file chunks 88, total chunks/vectors 291, Mapping 80건 실패 0, Risk 21건, Neo4j latest node 192/edge 558, Project Chat `local_openai/qwen2.5-coder-7b-instruct fallback=False`, AI Code Review 2개 commit 각 bug finding 1건, PL Briefing validation `valid`.
