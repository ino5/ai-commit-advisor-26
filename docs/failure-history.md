...
## 2026-07-22 - Docker read-only 저장소 mount로 외부 Git URL 등록이 실제 clone까지 이어지지 않았다

분류:

- Project onboarding
- Docker repository permissions
- Remote clone verification

관련 기능 및 문서:

- `src/ui/project_page.py`
- `src/services/git_remote_service.py`
- `src/utils/repo_path.py`
- `docker-compose.yml`
- [Git 저장소 운영 모델](git-repository-operating-model.md)

증상:

- 외부 링크에서도 프로젝트명과 Git remote URL을 저장할 수 있고 `서버 저장소 clone/fetch` 버튼이 보였지만, 현재 Docker 앱은 새 저장소를 실제로 만들 수 없었습니다.
- 기존 샘플 저장소의 Git 수집과 분석은 정상이라 외부 사용자 신규 등록과 기존 프로젝트 조회가 같은 수준으로 가능한 것처럼 보였습니다.

직접 원인:

- `C:/dev:/host-dev:ro` 하나가 기존 저장소 읽기와 앱 관리 clone 경로를 모두 담당했습니다.
- clone/fetch 서비스의 로컬 테스트는 쓰기 가능한 `tmp_path`를 사용해 Docker volume 권한 차이를 재현하지 않았습니다.

배경 또는 구조적 원인:

- 기존 저장소 보호를 위한 read-only 운영 정책과 remote URL clone/fetch 기능이 서로 다른 시점에 추가됐지만, 쓰기 권한이 필요한 경로를 별도로 분리하지 않았습니다.
- 프로젝트 정보 저장 성공, 저장소 읽기 성공, 저장소 생성 성공을 하나의 외부 사용자 시나리오로 교차 검증하지 않았습니다.

왜 사전 검증에서 놓쳤는지:

- Docker와 Cloudflare 검증은 이미 clone된 샘플 프로젝트의 화면과 저장 결과가 같은지 확인하는 데 집중했습니다.
- clone/fetch unit test는 서비스 로직과 dirty working tree guard를 검증했지만 Compose read-only mount에서의 신규 clone은 실행하지 않았습니다.
- 첫 관리형 UI 구현에서는 등록 방식 radio를 Streamlit form 안에 두어 선택 즉시 필드가 바뀌지 않았습니다. source와 unit test만으로는 이 rerun 차이를 확인할 수 없었고 실제 브라우저 snapshot에서 발견했습니다.

수정:

- 기존 `C:\dev`는 `/host-dev:ro`로 유지하고, `C:\dev\ai-commit-advisor-managed-repos`만 `/managed-repos:rw`로 추가했습니다.
- 프로젝트 설정에 `Git URL에서 가져오기`와 `서버에 이미 있는 저장소 사용`을 분리했습니다.
- 등록 방식 radio를 form 밖에 두어 선택 즉시 URL 입력과 서버 경로 입력이 전환되게 했습니다.
- 관리형 경로는 `project-<ID>`로 자동 배정하고 더 구체적인 관리형 path mapping을 우선 적용합니다.
- 관리형 remote는 허용된 공개 HTTPS host로 제한하고 credential prompt와 무제한 대기를 막았습니다.
- 기존 서버 경로도 `..` segment를 정규화한 뒤 허용 root 안인지 검사해 외부 입력이 path mapping 범위를 벗어나지 못하게 했습니다.

재발 방지 정책:

- Docker에서 파일을 쓰는 기능은 unit test 외에 실제 volume mode와 path mapping을 함께 검증합니다.
- read-only 분석 root를 쓰기 가능하게 완화하지 않고, 앱이 소유한 전용 write root를 별도로 둡니다.
- 외부 사용자 workflow는 폼 저장 성공과 후속 서버 작업 성공을 구분해 확인합니다.

남은 한계 또는 후속 확인:

- 인증, 사용자별 프로젝트 소유권, 저장공간 quota, 관리형 clone 자동 삭제는 아직 없습니다.
- 관리형 등록은 공개 HTTPS 저장소만 지원하며 private repository는 기존 서버 경로 운영을 사용합니다.

검증:

- `python -m pytest tests/test_repo_path.py tests/test_git_remote_service.py -q`: 16개 통과.
- `python -m pytest -q`: 186개 통과.
- `python -m compileall src app.py`: 통과.
- `docker compose config --quiet`: 통과.
- Docker mount 확인: `/host-dev RW=False`, `/managed-repos RW=True`.
- 실제 Docker 서비스 clone: `https://github.com/octocat/Hello-World.git`, branch `master`, HEAD `7fd1a60b01f9`; 생성한 smoke-test 폴더는 검증 후 삭제.
- Playwright UI 확인: 새 프로젝트에서 관리형 URL 방식이 기본으로 표시되고, `서버에 이미 있는 저장소 사용` 선택 즉시 서버 경로 field로 전환됨.

## 2026-07-21 - Nomic task profile 누락으로 한국어 질문이 영문 코드 근거를 찾지 못했다
...
- `nomic-embed-text-v1.5` vector를 대상으로 한국어로 결제, 재고, 쿠폰, 보고서, 주문 상태 구현 위치를 물으면 기대 Java service file이 `[56, 73, 50, 68, 31]`위에 머물렀습니다.
- 영어 질의는 상대적으로 나았지만 실제 사용자 질문인 한국어와 코드 식별자 사이의 검색 간극이 컸습니다.

직접 원인:

- 질의와 문서를 모두 같은 `embed_text` 경로로 보내 Nomic이 요구하는 `search_query:`와 `search_document:` 역할을 구분하지 않았습니다.
- 기존 v1.5 model은 현재 사용 사례의 한국어-영문 코드 교차 검색에 충분한 결과를 내지 못했습니다.
- 저장 key가 provider와 model 이름만 포함해, 같은 model에서 prefix 정책이 바뀌어도 이전 raw vector와 구분할 수 없었습니다.

배경 또는 구조적 원인:

- embedding 연결 검증은 HTTP 성공과 768차원만 확인했고, 모델별 input convention과 실제 한국어 retrieval ranking을 pass condition으로 두지 않았습니다.
- model upgrade를 chat model 크기 중심으로 보면서 retrieval model과 query/document asymmetry를 별도 평가하지 않았습니다.

왜 사전 검증에서 놓쳤는지:

- 기존 Project Chat 검증은 표준용어 query expansion과 identifier 보강을 함께 사용해 base embedding의 약점을 분리해 보지 못했습니다.
- 첫 A/B inline 실행은 PowerShell pipe encoding 때문에 한국어가 `?`로 바뀌었고 잘못된 ranking을 만들었습니다. 출력에서 깨진 질의를 확인해 해당 결과를 폐기하고 UTF-8로 다시 실행했지만, 입력 원문 자체를 검증하지 않았다면 잘못된 수치를 채택할 수 있었습니다.

수정 내용:

- 기본 model을 768차원의 `nomic-embed-text-v2-moe Q8_0`으로 바꿨습니다.
- query에는 `search_query:`, document에는 `search_document:`를 붙이고 model storage key를 `local_openai:text-embedding-nomic-embed-text-v2-moe:retrieval-v1`로 분리했습니다.
- 4,834개 모든 chunk에 새 vector를 생성했고 이전 vector는 새 검색에서 제외했습니다.
- OpenAI-compatible 배열 입력을 32개 단위로 보내 전체 재생성 시간을 줄였습니다.

재발 방지 규칙:

- embedding model을 도입할 때 model card의 query/document prefix, context, dimension을 구현과 테스트에서 각각 확인합니다.
- model 이름뿐 아니라 retrieval input policy가 바뀌어도 storage key를 새 profile로 분리합니다.
- 한국어 업무 질문과 영문 코드가 섞인 대표 질의를 고정하고 Top-K hit와 MRR을 전환 전후 같은 corpus에서 비교합니다.
- Windows inline benchmark는 `$OutputEncoding` 또는 UTF-8 source file을 사용하고, 실제 입력에 한글이 보존됐는지 결과에 함께 출력합니다.

남은 한계 또는 후속 확인 사항:

- 이번 ranking은 샘플 source 79개와 질의 5개의 소규모 진단입니다. 실제 프로젝트별 표준 평가 집합을 더 늘려야 합니다.
- 512-token context보다 긴 minified asset은 LM Studio가 잘라 처리합니다. 외부 압축 CSS/JavaScript를 source index에서 제외할지는 별도 정책 결정이 필요합니다.

검증 명령과 결과:

- `EmbeddingClient.test_connection()`: `retrieval-v1`, 768차원 성공.
- 동일한 UTF-8 한국어 질의 5개 A/B: v1.5 MRR `0.0197`, v2 MRR `0.3801`.
- 실제 Project Chat: source 6건 사용, `insufficient=False`, graph `completed`, 이번 호출 fallback 없음.

## 2026-07-21 - LM Studio의 `.part` 파일을 외부 도구로 이어받아 다운로드가 손상됐다
...
- LM Studio `lms get`, `lms import`
- `nomic-ai/nomic-embed-text-v2-moe-GGUF`
- `docs/setup-and-operations.md`

증상:

- `lms get` 호출이 timeout으로 끝났지만 LM Studio의 background download는 계속 진행됐습니다.
- 생성된 `.part` 파일을 일반 HTTP partial file로 보고 외부 `curl` resume를 적용하자 원격 원본 크기보다 큰 손상 파일이 만들어졌습니다.
- `lms import`도 호출 timeout 뒤 내부 복사와 index가 계속되어, 즉시 다시 시도하면 중복 작업을 만들 수 있는 상태였습니다.

직접 원인:

- LM Studio가 관리하는 `.part`의 내부 상태와 background writer를 확인하지 않고 다른 downloader가 같은 partial artifact를 이어받게 했습니다.
- CLI timeout을 작업 실패와 동일하게 해석했습니다.

배경 또는 구조적 원인:

- 다운로드 시작, 진행, checksum 검증, import, load 확인을 하나의 상태 흐름으로 관리하지 않았습니다.
- 큰 모델 파일에서 command timeout과 실제 child/background 작업 수명이 다를 수 있다는 운영 전제가 빠졌습니다.

왜 사전 검증에서 놓쳤는지:

- `.part` 확장자를 표준 HTTP range resume가 가능한 일반 파일로 가정했습니다.
- 파일 크기와 upstream SHA256은 외부 resume 뒤에 확인했습니다. 다행히 최종 경로로 이동하기 전에 크기 검사가 실패해 손상 파일이 load되지는 않았습니다.

수정 내용:

- 손상된 partial을 사용하지 않고 별도 임시 경로로 원본을 처음부터 다시 받았습니다.
- upstream 크기 `512225120 bytes`와 SHA256 `06e7a7e594a26985523c18383aba4aad39fe6e14f08ffc6ab5b554e1ccdc3cff`를 확인한 뒤 LM Studio에 import했습니다.
- 최종 model을 load하고 같은 hash를 다시 확인한 후, 약 1GB의 손상/중복 임시 파일만 Windows 휴지통으로 이동했습니다. 이 파일은 휴지통에서 복구할 수 있습니다.

재발 방지 규칙:

- LM Studio가 관리 중인 `.part` 파일에는 외부 resume 도구를 연결하지 않습니다.
- 외부 downloader가 필요하면 LM Studio 경로와 분리된 새 임시 파일로 처음부터 받고, upstream 크기와 SHA256을 확인한 뒤 import합니다.
- `lms get` 또는 `lms import`가 timeout이면 즉시 재시도하지 않고 process, file size 변화, `lms ls`, `lms ps`로 background 작업 상태를 먼저 확인합니다.
- 최종 load와 checksum 확인 전에는 partial이나 중복 파일을 삭제하지 않습니다.

남은 한계 또는 후속 확인 사항:

- 모델 설치 상태는 사용자 LM Studio 홈에 있어 repository test로 재현하지 않습니다.
- upstream revision이 바뀌면 크기와 SHA256도 달라질 수 있으므로 새 다운로드마다 원격 metadata를 다시 확인해야 합니다.

검증 명령과 결과:

- 최종 GGUF 크기 `512225120 bytes`, SHA256 일치.
- `lms ps --json`: `text-embedding-nomic-embed-text-v2-moe`, `Q8_0`, context 512 loaded.
- 실제 `/v1/embeddings`: 768차원 응답 성공.

## 2026-07-21 - 중단한 재임베딩 자식 프로세스가 계속 실행되어 중복 vector가 생겼다
...
- `src/rag/vector_store.py`
- `vector_items`
- `ROADMAP.md` Candidate Task `Enforce vector uniqueness per embedding profile`

증상:

- 단건 재임베딩이 오래 걸려 실행 cell을 종료한 뒤 32개 batch 방식으로 다시 시작했습니다.
- 종료된 것으로 본 첫 Python 자식 프로세스가 잠시 계속 실행되어 두 작업이 같은 model key를 동시에 처리했고, 새 model에 총 4,961개 row와 4,834개 고유 chunk가 남았습니다.
- 127개 chunk가 같은 `(chunk_id, embedding_model)` vector를 2개씩 가졌습니다.

직접 원인:

- orchestration cell 종료가 PowerShell 아래 Python child process 종료까지 보장한다고 가정했습니다.
- 두 worker가 vector 존재 여부를 확인한 뒤 insert하는 사이를 DB constraint가 막지 못했습니다.

배경 또는 구조적 원인:

- `VectorStore`의 중복 방지는 애플리케이션 조회에만 의존하고 `vector_items`에는 `(chunk_id, embedding_model)` unique constraint가 없습니다.
- 장시간 운영 명령을 중단한 뒤 child process와 DB 증가가 멈췄는지 확인하는 절차가 없었습니다.

왜 사전 검증에서 놓쳤는지:

- 실행 도구가 `terminated`를 반환한 직후 한 번의 vector count만 보고 두 번째 작업을 시작했습니다.
- re-embedding 완료 조건을 total row 수로만 보았다면 duplicate를 발견하지 못했을 수 있습니다. 이후 total과 distinct chunk를 대조해 확인했습니다.

수정 내용:

- 127개 duplicate group의 embedding 값이 각 group 안에서 완전히 동일한지 먼저 확인했습니다.
- 새 model key 범위에서 최초 row 하나를 남기고 값이 같은 추가 row 127개만 삭제했습니다. 다른 model vector와 `document_chunks`는 변경하지 않았습니다.
- 최종 상태를 `4,834 total vectors = 4,834 distinct chunks`, missing 0으로 확인했습니다.
- 이후 재생성은 32개 API batch와 batch 단위 DB commit을 사용해 남은 2,670개를 약 65초에 실패 없이 처리했습니다.

재발 방지 규칙:

- re-embedding 작업을 중단한 뒤에는 parent 결과만 믿지 않고 `Win32_Process`, DB row 증가, LM Studio queue가 모두 멈췄는지 확인합니다.
- 같은 model key에 대해 두 re-embedding worker를 동시에 실행하지 않습니다.
- 완료 검증은 `count(*)`, `count(distinct chunk_id)`, missing count를 함께 비교합니다.
- `(chunk_id, embedding_model)` unique constraint와 충돌-safe insert는 별도 schema task로 구현합니다.

남은 한계 또는 후속 확인 사항:

- DB unique constraint가 아직 없어 다른 경로에서 concurrent 작업을 시작하면 같은 문제가 재발할 수 있습니다.
- batch 요청 실패는 단건 처리로 격리하지만 작업 queue와 progress persistence는 아직 없습니다.

검증 명령과 결과:

- duplicate 정리 전: total 4,961, distinct 4,834, duplicate group 127.
- group별 vector equality 확인: mismatch 0, exact duplicate 127개 삭제.
- 정리 후: total 4,834, distinct 4,834, missing 0.
- 전체 `pytest -q`: 180 passed.

## 2026-07-21 - model 전환 검증이 preflight의 최신 저장 결과와 vector 수 기준을 바꿨다

분류:

- Demo preflight
- Local AI verification side effect
- Multi-model vector accounting

관련 기능 및 문서:

- `scripts/demo_preflight.ps1`
- `scripts/run_local_ai_verification.py`
- `vector_items`, `code_review_results`

증상:

- v1.5 vector를 보존한 채 v2 vector를 추가한 뒤 preflight가 source 79건에 vector 158건이라고 계산해 실패했습니다.
- 실제 연결 검증을 위해 기본 `latest_commit` Code Review를 실행하자 bug finding 0건인 HEAD review가 가장 최근 결과가 됐고, preflight가 요구하는 시연 기준 `2325182` bug finding 1건을 가렸습니다.

직접 원인:

- preflight가 active embedding storage key를 거르지 않고 프로젝트의 모든 `VectorItem` row를 셌습니다.
- `run_local_ai_verification.py`의 Code Review는 read-only probe가 아니라 정상 review 결과를 DB에 저장하며, target을 지정하지 않으면 latest commit을 사용합니다.

배경 또는 구조적 원인:

- 기존 preflight는 프로젝트에 active embedding model이 하나만 저장된다고 가정했습니다.
- 연결/형식 검증과 시연용 최신 저장 결과 준비를 같은 명령으로 수행했습니다.

왜 사전 검증에서 놓쳤는지:

- model migration 뒤에도 source count와 전체 vector count가 같을 것이라고 가정했습니다.
- live verification 성공 여부만 확인하고, 최신 저장 결과를 고정하는 preflight를 바로 다시 실행하지 않았습니다.

수정 내용:

- preflight가 `EmbeddingClient.embedding_model_name`으로 active profile을 계산하고 해당 model key의 `count(distinct chunk_id)`만 source count와 비교하도록 바꿨습니다.
- `2325182 Relax partner payment validation for pilot channel`을 target으로 실제 Code Review를 다시 실행해 bug finding 1건을 최신 저장 결과로 복원했습니다.

재발 방지 규칙:

- 여러 embedding profile을 보존하는 환경의 준비 상태는 전체 vector row가 아니라 active storage key의 고유 chunk coverage로 판단합니다.
- 시연 DB에서 local verification Code Review를 실행할 때는 의도한 `--code-review-target`과 `--code-review-ref`를 명시합니다.
- live 검증 뒤에는 preflight를 다시 실행해 최신 Project Chat/Code Review와 source/vector 기준이 유지됐는지 확인합니다.
- 연결과 schema만 확인할 때 저장 결과를 만들지 않는 verification mode는 별도 roadmap 후보로 관리합니다.

남은 한계 또는 후속 확인 사항:

- 현재 local verification의 PL Briefing, Project Chat, Code Review, Mapping은 정상 기능처럼 DB를 변경합니다.
- active profile을 project 설정으로 별도 저장하지 않고 현재 process 환경에서 계산하므로, 컨테이너와 host 설정이 다르면 각 surface에서 따로 확인해야 합니다.

검증 명령과 결과:

- `scripts/demo_preflight.ps1 -ProjectId 2716`: active profile vector `79/79`, Code Review target `2325182`, bug finding 1, `FAIL=0`, `WARN=0`.
- Docker app health `200`, embedding profile `local_openai:text-embedding-nomic-embed-text-v2-moe:retrieval-v1`, dimension 768.
...
- 동일 저장소를 여러 프로젝트에서 동시에 쓰는 문제는 2026-07-22 revision `20260722_0011`과 project-scoped Git Sync로 해소됐습니다. 이 항목의 runtime surface 통일 교훈은 계속 적용합니다.
...
> 아래 격리 DB 조치는 전체 재현 중 기존 데이터를 보호한 당시 기록입니다. 근본 원인은 2026-07-22 revision `20260722_0011`, project-scoped Git Sync, 교차 저장소 회귀 테스트로 수정했습니다.
...
- `src/db/models.py`
- `migrations/versions/20260722_0011_scope_git_commit_hash_by_project.py`
- `docs/end-to-end-demo-evidence-2026-07-21.md`
...
- 당시 전체 재현은 별도 PostgreSQL DB와 Streamlit 인스턴스에서 수행해 기존 project 197을 변경하지 않았습니다.
- revision `20260722_0011`에서 `uq_git_commits_commit_hash`를 제거하고 `(project_id, commit_hash)` unique constraint만 유지했습니다.
- Git Sync가 현재 프로젝트 안에서만 같은 hash를 찾도록 바꾸고, 다른 프로젝트의 `GitCommit.project_id`를 다시 지정하던 로직을 삭제했습니다.
- Commit Impact의 DB ID 조회도 `project_id`를 함께 확인하게 바꿨습니다.
- PostgreSQL commit/file·Mapping, RAG chunk/vector, Neo4j payload가 같은 hash를 두 프로젝트에서 독립적으로 유지하는 통합 테스트를 추가했습니다.
- 기본 DB의 `Sample Shop Demo (#1)`와 `Sample Shop Demo (github) (#393)`에 같은 48개 history를 수집하고 RAG/vector와 Neo4j graph를 별도로 생성했습니다.

재발 방지 규칙:

- Git commit 중복 여부는 항상 `(project_id, commit_hash)`로 판단하며, hash만으로 기존 행의 프로젝트 소유권을 바꾸지 않습니다.
- commit DB ID를 받는 화면·서비스는 선택한 `project_id`도 함께 확인합니다.
- Git identity를 바꿀 때는 관계형 FK뿐 아니라 RAG `DocumentChunk`/`VectorItem`, Neo4j node ID와 cleanup까지 같은 회귀 테스트에서 확인합니다.
- 이전 version에서 같은 저장소를 중복 수집한 DB는 migration만으로 정상 상태를 추정하지 않습니다. 백업 후 관련 프로젝트의 분석 데이터 초기화와 Git/Mapping/RAG/graph 재생성을 수행합니다.

남은 한계 또는 후속 확인 사항:

- 같은 저장소를 프로젝트별로 복제 저장하므로 commit/file, commit 기반 RAG 데이터 사용량이 프로젝트 수에 비례해 늘어납니다.
- 서로 다른 프로젝트에 같은 hash가 저장된 뒤 revision `20260722_0011`을 downgrade하려면 전역 unique constraint를 복원하기 전에 중복 프로젝트 데이터를 명시적으로 정리해야 합니다.
- migration은 과거에 이미 이동한 소유권과 stale AI 결과를 자동 복구하지 않습니다.

검증 명령과 결과:

- migration 전 PostgreSQL custom dump: `C:\dev\ai-commit-advisor-backups\ai_commit_advisor_pre_project_scope_20260722.dump`, 435,003 bytes.
- `alembic current`: `20260722_0011 (head)`; `git_commits` unique constraint는 `uq_git_commits_project_hash (project_id, commit_hash)` 1개입니다.
- 신규 통합 테스트와 Git/Mapping/RAG/Neo4j 관련 focused test: 46개 통과.
- `PGVECTOR_DIMENSION=768`, `LLM_PROVIDER=mock`, `EMBEDDING_PROVIDER=mock` 기준 전체 test: 217개 통과.
- 기본 DB project `1`과 `393`: commit 각각 48건, 변경 파일 각각 106건, 공통 hash 48개. 재수집은 저장 0건, 현재 프로젝트 중복 48건, 오류 0건입니다.
- RAG source/vector는 두 프로젝트 각각 79/79이며 실제 `text-embedding-nomic-embed-text-v2-moe` 768차원 embedding 생성 오류가 0건입니다.
- 같은 query의 RAG 상위 5개 결과는 project `1`과 `393` 요청에서 각각 자기 프로젝트 chunk만 반환했고, cross-project Mapping/commit chunk/commit_file chunk 조회 결과는 모두 0건입니다.
- 동일 HEAD의 PostgreSQL ID는 project `1`에서 `48`, project `393`에서 `459`; Neo4j node ID는 각각 `p1:commit:221eb9...`, `p393:commit:221eb9...`입니다.
- project `393` Neo4j 전체 동기화는 node 202개, edge 533개, 오류 0건이며 project `1` node 213개가 유지됐습니다.
...
## 2026-06-30 - PPT 선 도형의 음수 너비 때문에 PowerPoint가 파일을 손상으로 인식했다
...
- `outputs/ai-commit-advisor-architecture-simple-editable.pptx`
- `outputs/ai-commit-advisor-architecture-simple-ppt.svg`
- `AI_CHANGELOG.md` 항목 `PPT용 단순 아키텍처 선 그림 추가`

### 증상

편집 가능한 native shape 기반 아키텍처 PPTX를 생성한 뒤 artifact-tool preview에서는 슬라이드가 렌더링됐지만, PowerPoint COM으로 최종 PPTX를 열 때 `The file or directory is corrupted and unreadable` 오류가 발생했습니다. 파일은 ZIP으로 열렸고 artifact-tool inspect도 가능했지만, 실제 PowerPoint에서는 손상 파일로 처리됐습니다.

### 직접 원인

오른쪽 `Local LLM`에서 왼쪽 graph bus로 이어지는 점선을 `x1 > x2` 좌표로 생성해 PPT 내부 shape bbox가 음수 너비로 저장됐습니다. PowerPoint는 이 음수 폭 line shape를 복구하지 못하고 파일 전체를 손상으로 판단했습니다.

### 배경 또는 구조적 원인

SVG에서는 오른쪽에서 왼쪽으로 그은 선도 정상 렌더링되지만, PPTX native shape에서는 line position이 음수 width/height를 만들 수 있습니다. artifact-tool preview만 보면 이런 PowerPoint 호환성 문제가 드러나지 않을 수 있습니다.

### 사전 검증에서 놓친 이유

처음에는 artifact-tool의 PNG preview만 확인했고, 최종 PPTX를 PowerPoint 자체로 열어보는 검증을 뒤늦게 수행했습니다. 또한 inspect 출력의 `bbox`에서 음수 값을 확인하는 절차가 없었습니다.

### 수정 내용

- 문제 선을 왼쪽에서 오른쪽으로 생성하도록 좌표 순서를 바꿨습니다.
- `outputs/ai-commit-advisor-architecture-simple-editable.pptx.inspect.ndjson`에서 음수 bbox가 더 이상 나오지 않는지 확인했습니다.
- PowerPoint COM으로 최종 PPTX 열기와 1장 PNG export를 재검증했습니다.

### 재발 방지 규칙

- PPT native shape로 선을 만들 때는 `width`와 `height`가 음수가 되지 않도록 좌표 순서를 정규화합니다.
- artifact-tool preview가 성공해도 최종 PPTX는 PowerPoint COM 또는 실제 PowerPoint로 한 번 열고 export해 확인합니다.
- diagram PPTX를 생성한 뒤 inspect/layout에서 음수 bbox나 비정상 line bounds가 없는지 확인합니다.

### 남은 한계 또는 후속 확인 사항

현재 PPTX는 PowerPoint에서 열리고 export되지만, connector 자동 라우팅은 PowerPoint와 artifact-tool preview에서 미세하게 다르게 보일 수 있습니다. 발표자료에 넣기 전에는 최종 PPT에서 직접 열어 필요한 선 위치를 한 번 더 손으로 조정할 수 있습니다.

### 검증 명령과 결과

- `Select-String -Path outputs\ai-commit-advisor-architecture-simple-editable.pptx.inspect.ndjson -Pattern 'bbox\":\[[^\]]*-'` 결과 음수 bbox 없음.
- PowerPoint COM으로 `outputs\ai-commit-advisor-architecture-simple-editable.pptx` 열기 성공 및 `slides=1` 확인.
- PowerPoint COM `Slides.Item(1).Export(..., 'PNG', 1280, 720)` 성공.
- export된 `slide-01.png`를 시각 확인해 `분석 결과 화면` 노드가 제거되고 기본 선/아이콘 구성이 렌더링됨을 확인했습니다.

## 2026-06-29 - 내부 공유용 PPT 보완 방향이 Use Case 원문보다 한계 설명에 치우쳤다

분류:

- Presentation content quality
- Use Case source handling
- Agent planning

관련 기능 및 문서:

- `outputs/ai-use-case-team-share.pptx`
- `outputs/backups/ai-use-case-team-share.before-rewrite.20260629-194802.pptx`
- `docs/ai-use-case-brief-reference.md`
- `ROADMAP.md` 항목 `AI Use Case internal share deck quality rewrite`
- `AI_CHANGELOG.md` 항목 `AI Use Case 참고문서 고정과 내부 공유용 PPT 방향 재정의`

### 증상

초기 내부 공유용 PPT는 기능 이름과 화면을 짧게 나열했지만, Project Chat의 실제 질문/답변, GraphRAG 근거, AI Code Review 결과가 충분히 드러나지 않았습니다. 이후 보완 방향을 잡을 때도 `한계`와 `AI Progress 세부 기준` 설명을 먼저 앞세워, 사용자가 제공한 Use Case 원문의 고객 Pain Point와 기대 효과가 발표 구조의 중심에 놓이지 않았습니다.

추가로 사용자가 의도한 발표 성격은 `AX Use Case` 과제 결과의 내부 공유였습니다. 실제 운용이나 상용 도입을 설득하는 문서가 아니라 "AI를 이용해 이런 프로젝트를 했다"는 내부 보고였는데, 이를 명확히 고정하지 않아 발표 톤이 과하게 방어적이고 운영 검토 문서처럼 흐를 위험이 생겼습니다.

### 직접 원인

상위 폴더의 Use Case 텍스트를 읽고 요약은 했지만, repo 안의 기준 문서로 고정하지 않았습니다. 그 결과 PPT를 보완할 때 원문에서 강조한 `진척도 관리의 불확실성`, `업무 편중`, `리뷰 리소스 부족`, `1MM 이상 예상` 같은 발표 기준보다 내부 구현 한계와 산식 설명이 과하게 커졌습니다.

### 배경 또는 구조적 원인

이 프로젝트에는 AI Progress, GraphRAG, Code Review, Dashboard, Risk Analysis 같은 기능 설명 문서가 많습니다. 기준 원문을 먼저 고정하지 않으면, 발표자료가 사용자가 보여주려는 Use Case보다 시스템 내부 기능 설명이나 방어적 한계 설명으로 흐르기 쉽습니다.

### 사전 검증에서 놓친 이유

PowerPoint 열기, preview export, 문구 검색은 수행했지만, "이 덱이 사용자가 준 Use Case 문서의 Pain Point와 기대 효과를 중심으로 말하고 있는가"를 별도 기준으로 검증하지 않았습니다. 또한 실제 대화 질문/답변이 장표에 보이는지 확인하지 않아, Project Chat/GraphRAG 가치가 스크린샷 장식처럼 약해졌습니다.

### 수정 내용

- 기존 PPT를 `outputs/backups/`에 백업했습니다.
- 상위 폴더의 Use Case 텍스트를 `docs/ai-use-case-brief-reference.md`로 추가하고, 발표자료 해석 기준을 함께 기록했습니다.
- `CONTEXT.md`에 `AX Use Case`를 내부 AI 활용 과제 결과 공유 맥락으로 정의했습니다.
- `docs/ai-use-case-brief-reference.md`에 발표 성격을 추가해 실제 운영 전환 제안서가 아니라 내부 공유용 과제 결과 보고임을 명시했습니다.
- Roadmap의 보완 작업을 `AI Use Case internal share deck quality rewrite`로 다시 열고, AI Progress는 세부 산식보다 진척도 관리 불확실성을 줄이는 화면 근거로 설명하도록 체크리스트를 수정했습니다.

### 재발 방지 규칙

- 사용자가 PPT의 기준이 되는 텍스트나 brief를 제공하면, 먼저 repo 안에 참고문서로 고정하고 그 문서의 Pain Point, 목적, 기대 효과를 slide acceptance 기준으로 삼습니다.
- 발표 대상과 발표 성격을 먼저 고정합니다. 내부 과제 결과 공유라면 실제 운영 전환 계획, 리스크 방어 논리, 한계 설명보다 `AX Use Case로 무엇을 만들었고 AI를 어디에 적용했는지`가 본문 중심입니다.
- 내부 공유용 PPT는 기능 목록보다 실제 질문, 답변, 근거, 발견 결과를 먼저 보여줍니다.
- 한계나 산식 설명은 Use Case 본문 메시지를 약하게 만들지 않도록 마지막 보조 장표나 발표자 설명 수준으로 제한합니다.

### 남은 한계 또는 후속 확인 사항

새 PPTX 전면 보완본은 `outputs/ai-use-case-team-share.pptx`로 다시 생성했습니다. 이후 같은 종류의 내부 공유용 PPT를 만들 때는 `docs/ai-use-case-brief-reference.md`처럼 기준 brief를 먼저 고정하고, 기술 구성도와 실제 화면 근거가 brief의 Pain Point와 연결되는지 검토해야 합니다.

### 검증 명령과 결과

- `Get-Content -Path C:\dev\와이어-AI커밋분석기반프로젝트자원관리서비스.txt -Encoding UTF8`로 원문 확인.
- `docs/ai-use-case-brief-reference.md`에 원문과 발표자료 해석 기준 추가.
- `ROADMAP.md`의 `AI Use Case internal share deck quality rewrite` 체크리스트 갱신.
- `outputs/ai-use-case-team-share.pptx`를 13장으로 전면 재작성.
- PowerPoint COM으로 최종 PPTX 열기 성공, 슬라이드 13장 확인, 전체 슬라이드 PNG export 성공.

## 2026-06-29 - PPT screenshot을 frame에 강제로 맞춰 화면 비율이 깨졌다
...
- `outputs/ai-use-case-team-share.pptx`
- `outputs/backups/ai-use-case-team-share.before-aspect-fix.20260629-202340.pptx`
- `ROADMAP.md` 항목 `AI Use Case internal share deck aspect and polish pass`
- `AI_CHANGELOG.md` 항목 `AI Use Case 내부 공유용 PPT 이미지 비율과 품질 보정`

### 증상

내부 공유용 PPT에서 6페이지와 7페이지의 screenshot이 납작하게 보였습니다. 실제로는 원본 screenshot crop 비율과 PPT frame 비율이 달랐는데, PPT 생성 스크립트가 이미지를 지정된 `w/h`에 그대로 맞추면서 가로로 늘어난 상태였습니다.

추가 preview 확인 중 8페이지 하단 설명 겹침, 11페이지 GraphRAG bullet 겹침과 그래프 하단 잘림도 함께 발견했습니다.

### 직접 원인

PPT 생성 스크립트의 `image()` helper가 source image aspect ratio를 계산하지 않고 `slide.addImage({ x, y, w, h })`로만 삽입했습니다. 일부 screenshot은 세로로 긴 앱 화면에서 잘라낸 crop이었는데, 장표의 넓은 frame에 강제로 맞추면서 화면 UI가 찌그러졌습니다.

### 배경 또는 구조적 원인

Streamlit 화면 screenshot은 대부분 세로로 길고, 발표 슬라이드는 16:9에 가까운 넓은 프레임을 사용합니다. 원본 비율을 유지하려면 장표 frame을 screenshot 비율에 맞추거나, 반대로 screenshot을 frame 비율에 맞게 다시 crop해야 합니다. 이 확인을 개별 slide preview에서만 보다가 전체 crop 비율 계산을 하지 않아 문제가 남았습니다.

### 사전 검증에서 놓친 이유

PowerPoint 열기와 전체 slide export는 통과했지만, export 성공은 이미지가 정상 비율로 보인다는 뜻은 아닙니다. contact sheet에서는 흐름만 확인했고, screenshot 내부 UI 비율이 자연스러운지까지 6/7페이지를 크게 확대해 보지 않았습니다.

### 수정 내용

- 현재 PPT를 `outputs/backups/ai-use-case-team-share.before-aspect-fix.20260629-202340.pptx`로 백업했습니다.
- 6페이지와 7페이지를 포함한 주요 screenshot crop을 PPT frame 비율과 맞게 다시 생성했습니다.
- 제작 메모처럼 보이던 `PPT에 넣을 메시지`, `이 장표의 역할` 문구를 발표용 문구로 교체했습니다.
- 8페이지 하단 설명을 panel로 정리하고 줄바꿈을 줄였습니다.
- 11페이지 GraphRAG bullet과 graph crop을 조정했습니다.

### 재발 방지 규칙

- PPT에 screenshot을 넣을 때는 원본 crop 비율과 slide frame 비율을 비교한 뒤 삽입합니다.
- 화면을 frame에 강제로 늘리는 대신, frame 비율에 맞춰 screenshot을 다시 crop하거나 frame 자체를 원본 비율에 맞춥니다.
- PowerPoint export 성공만으로 QA를 끝내지 말고, contact sheet와 핵심 slide full-size preview에서 UI 비율, 텍스트 겹침, 제작 메모 잔존 여부를 확인합니다.

### 남은 한계 또는 후속 확인 사항

이번 보정은 기존 Application Preview screenshot을 재활용한 crop 조정입니다. 더 높은 품질이 필요하면 각 기능 화면을 발표용 viewport로 다시 캡처하는 방식이 더 낫습니다.

### 검증 명령과 결과

- PowerPoint COM으로 `outputs\ai-use-case-team-share.pptx` 열기 성공, 슬라이드 13장 확인.
- PowerPoint COM `Slides.Export(..., 'PNG', 1280, 720)`로 전체 slide preview 렌더링 성공.
- `contact-sheet.png`와 6, 7, 8, 11페이지 full-size preview를 확인해 screenshot 비율, 하단 설명, GraphRAG crop을 점검했습니다.

## 2026-06-29 - 내부 공유용 PPT 생성 중 presentation tooling 전제가 맞지 않았다
...
- `outputs/ai-use-case-team-share.pptx`
- `AI_CHANGELOG.md` 항목 `AI Use Case 내부 공유용 PPT 작성`
- presentation skill의 `@oai/artifact-tool` workspace setup

### 증상

팀 내부 공유용 PPT를 만들기 위해 presentation skill의 표준 경로인 `@oai/artifact-tool` workspace setup을 실행했지만, 로컬 Codex runtime cache의 예상 위치에 package가 없어 setup이 실패했습니다. 대체로 `pptxgenjs`를 사용해 PPTX를 생성한 뒤에도 처음 만든 파일은 PowerPoint에서 열리지 않았습니다.

### 직접 원인

- `C:\dev\ai-commit-advisor\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\node_modules\@oai\artifact-tool` 경로에 `package.json`이 없어 artifact-tool setup script가 package 검증을 통과하지 못했습니다.
- `pptxgenjs`의 `addTable`로 만든 native table XML이 이 환경의 PowerPoint에서 `corrupted and unreadable` 오류를 만들었습니다.

### 배경 또는 구조적 원인

presentation skill은 bundled artifact-tool package가 로컬 runtime cache에 존재한다는 전제를 둡니다. 이 전제가 깨지면 skill 문서의 표준 생성·렌더링 경로를 그대로 사용할 수 없습니다. 또한 PPTX 생성 라이브러리는 최소 텍스트/이미지 파일은 정상이어도 table 같은 일부 OpenXML 객체에서 PowerPoint 호환성 문제가 뒤늦게 드러날 수 있습니다.

### 사전 검증에서 놓친 이유

환경 package 존재 여부는 실제 deck workspace setup을 실행하기 전까지 확인되지 않았습니다. `pptxgenjs` 최소 샘플과 이미지/화살표 샘플은 PowerPoint에서 열렸지만, 부록 표까지 포함한 전체 파일을 열어보기 전에는 `addTable` 문제가 드러나지 않았습니다.

### 수정 내용

- `@oai/artifact-tool` setup 실패 후 JavaScript 기반 `pptxgenjs`로 PPTX를 생성했습니다.
- PowerPoint에서 열리지 않는 원인을 좁히기 위해 최소 PPT, 이미지 삽입, 화살표, table을 순차 검증했습니다.
- `pptxgenjs.addTable` 사용을 중단하고 부록 표를 사각형과 텍스트 grid로 직접 구성했습니다.
- PowerPoint COM으로 최종 PPTX를 열고 전체 슬라이드를 PNG로 export해 실제 렌더링 결과를 확인했습니다.

### 재발 방지 규칙

- presentation 작업에서는 deck 생성 후 즉시 PowerPoint 또는 LibreOffice로 파일 열기 검증을 먼저 수행합니다.
- artifact-tool setup이 실패하면 package 경로 문제를 기록하고, 대체 생성 도구를 쓸 때도 최종 PPTX 열기와 slide export 검증을 필수로 수행합니다.
- `pptxgenjs`를 fallback으로 사용할 때는 `addTable` 대신 shape/text grid를 우선 사용하거나, table이 꼭 필요하면 별도 최소 파일로 PowerPoint 호환성을 먼저 확인합니다.

### 남은 한계 또는 후속 확인 사항

이번 PPT는 artifact-tool의 layout/overlap inspector가 아니라 PowerPoint 렌더링 preview와 시각 확인으로 QA했습니다. 이후 artifact-tool package가 복구되면 표준 skill 경로로 생성·검증하는 편이 더 낫습니다.

### 검증 명령과 결과

- `node ...\container_tools\setup_artifact_tool_workspace.mjs --workspace ...` 실패: bundled `@oai/artifact-tool` package `package.json` 누락.
- `npm view @oai/artifact-tool version` 실패: public npm registry에 package 없음.
- `npm install pptxgenjs@4.0.1 --save-exact` 성공.
- PowerPoint COM으로 최소 PPT, 이미지/화살표 샘플은 열림 확인.
- PowerPoint COM으로 `pptxgenjs.addTable` 샘플은 `corrupted and unreadable` 실패 확인.
- PowerPoint COM으로 `outputs\ai-use-case-team-share.pptx` 열기 성공, 슬라이드 10장 확인, 전체 슬라이드 PNG export 성공.

## 2026-06-21 - PL 가치 설명을 새 Cockpit 화면으로 과잉 구현했다
...
- `outputs/ai-commit-advisor-final-report.pptx`
- `AGENTS.md` 항목 `AI-Sounding Wording Review`
- `docs/engineering-decisions.md` 항목 `사용자-facing 산출물은 AI스러운 문구를 종료 전 필수 점검한다`
- `AI_CHANGELOG.md` 항목 `AI Use Case 기술 상세 마스터덱 전면 재작성`

### 증상

사용자가 "AI Use Case"와 "실제 SI 프로젝트에서 개발 리더가 개발자 관리 시 도움을 받는 관점"을 보여주고 싶다고 했는데, 이를 `PL 관리 Cockpit`이라는 새 화면과 PPT의 노골적인 PL/Cockpit 문구로 구현했습니다. 결과적으로 제품 가치 설명이 아니라 데모를 위해 급조한 AI 화면처럼 보였고, 사용자가 충격을 받을 정도로 방향이 어긋났습니다.

후속 수정에서도 기존 PPT를 버리고 13장 요약본으로 줄이면서 반대 방향의 문제가 생겼습니다. 기술 아키텍처, LLM Mapping, RAG/pgvector, Neo4j GraphRAG, AI telemetry, Resource Metrics 같은 실제 AI 적용 내용이 충분히 드러나지 않아 "AI Use Case라는 느낌"이 약해졌습니다.

### 직접 원인

요청의 핵심은 기존 기능을 어떻게 보여줄지와 결과서 문구를 어떻게 정리할지였는데, "대형 신규 기능 변경도 고려"라는 말을 새 화면 추가 허가로 과하게 해석했습니다. 또한 `PL`이라는 역할명을 제품 표면과 PPT 제목에 직접 노출하면서, 사용자가 원한 자연스러운 관리 가치 설명보다 내부 발표용 키워드가 앞섰습니다.

### 배경 또는 구조적 원인

기존 Dashboard, AI Resource Radar, Risk Analysis, AI Progress, Project Chat, AI Code Review만으로도 담당자별 업무 부담과 리스크 확인 가치를 설명할 수 있었습니다. 그런데 새 화면을 추가하면 Use Case가 더 선명해질 것이라는 가정이 앞섰고, 사용자-facing 문구가 실제 제품 화면처럼 자연스러운지보다 기능 구조를 빨리 만드는 데 치우쳤습니다.

### 사전 검증에서 놓친 이유

컴파일, 테스트, screenshot, PPT QA는 통과했지만 "이 화면과 문구가 사용자가 보여주려는 제품 톤에 맞는가"를 별도 검증하지 않았습니다. 특히 새 화면 이름, PPT 제목, 반복되는 `PL`, `Cockpit`, `action queue` 표현이 AI스럽거나 급조된 느낌을 주는지 확인하지 않았습니다.

### 수정 내용

- `PL 관리 Cockpit` 화면, service, test, screenshot, navigation, screenshot scenario, 관련 문서 추가분을 제거했습니다.
- 결과서 PPT는 기존 8장 PPT를 참고하지 않고, 텍스트 계획서는 사업 문제와 기대효과 기준으로만 사용했습니다.
- 13장 요약본은 폐기하고 `docs/architecture.md`, `docs/ai-technical-overview.md`, `docs/feature-guide.md`, README와 현재 앱 screenshot 근거를 바탕으로 47장 기술 상세 마스터덱으로 전면 재작성했습니다.
- 작성자가 추가한 `PL`/`Cockpit` 전면 문구와 새 화면 이미지를 제거했고, 금지어가 보이는 기존 screenshot도 결과서에서 제외했습니다.
- `AGENTS.md`에 `AI-Sounding Wording Review`를 추가해 사용자-facing 문서와 발표자료를 마무리하기 전에 AI스러운 문구를 반드시 확인하도록 했습니다.
- 이번 정책 변경을 `docs/engineering-decisions.md`에 기록했습니다.

### 재발 방지 규칙

- 사용자가 "관점"이나 "보여주고 싶은 것"을 말했을 때 곧바로 새 화면을 만들지 말고, 먼저 기존 제품 표면에서 자연스럽게 설명할 수 있는지 확인합니다.
- 사용자가 AI Use Case 결과서를 요청하면 사업 요약만 만들지 말고, 실제 AI 적용 구조와 검증 근거를 충분히 담은 마스터덱을 먼저 만든 뒤 제출용 장표를 골라내게 합니다.
- 역할명이나 발표 키워드는 제품 제목과 메뉴에 직접 박지 말고, 필요한 경우 설명 문장 안에서만 제한적으로 사용합니다.
- PPT/문서/화면 문구는 종료 전 `AI-Sounding Wording Review`를 수행하고, `PL`, `Cockpit`, `action queue`처럼 내부 발표용으로 보이는 단어가 과하게 남아 있지 않은지 확인합니다.

### 남은 한계 또는 후속 확인 사항

기존 앱 내부의 오래된 기능명까지 전면 개편하지는 않았습니다. 다만 이번 결과서에서는 해당 표현이 보이는 screenshot을 쓰지 않았고, 향후 사용자가 제품 용어 자체도 자연스럽게 바꾸길 원하면 별도 UX copy 정리 작업으로 다룹니다.

### 검증 명령과 결과

- `rg -n "PL 관리 Cockpit|PL Management Cockpit|pl_management|pl-management|개발자 집중관리|프로그램 액션 큐" app.py scripts README.md docs src tests ROADMAP.md -g "!docs/failure-history.md"` 결과 없음. Changelog와 failure-history에는 제거 기록으로만 남깁니다.
- PPT inspect/layout에서 `PL`, `Cockpit`, `pl-management`, `개발자 집중관리`, `프로그램 액션`, `action queue` 검색 결과 없음.
- PPT inspect/layout에서 `혁신적인|강력한|원활한|향상된 사용자 경험|AI 기반으로 자동화|이를 통해|최첨단|놀라운|완벽한` 검색 결과 없음.
- PPT 슬라이드 47장 확인, `LLM`, `Embedding`, `RAG`, `pgvector`, `Neo4j`, `GraphRAG`, `Mapping`, `AI Progress`, `Risk Analysis`, `AI Code Review`, `ai_invocation_logs`, `resource_metric_snapshots`, `Knowledge Graph`, `Project Chat` 포함 확인.
- preview 주요 슬라이드 1, 9, 15, 28, 32, 34, 47 시각 확인.
- `.\.venv\Scripts\python.exe -m pytest tests\test_documentation_images.py -q` 통과.
- `git diff --check` 통과. Windows line-ending 경고만 표시.
