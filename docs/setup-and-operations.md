...
6. embedding model `text-embedding-nomic-embed-text-v1.5`를 함께 로드하고 `/v1/embeddings` 요청에 응답하는지 확인합니다.
...
Ownership label이 붙은 전용 Tunnel이나 legacy `ai_commit_advisor_quick_tunnel`이 실행 중이면 기존 주소를 재사용하고 외부 health를 다시 확인합니다. 전용 container가 중지된 상태이거나 실행 중인 Tunnel이 없으면 새 Quick Tunnel을 만들므로 새 주소가 발급됩니다. 동시에 두 Tunnel이 실행 중이면 주소를 임의로 선택하지 않고 오류로 중단합니다.
...
`--stop-app`도 PostgreSQL, Neo4j, Docker volume은 중지하거나 삭제하지 않습니다. Legacy `ai_commit_advisor_quick_tunnel`은 자동 제거 대상이 아니므로 `status`가 알려준 이름을 확인한 뒤 `docker stop ai_commit_advisor_quick_tunnel`로 명시적으로 중지합니다. 시연이 끝나면 임시 URL을 더 이상 사용할 수 없도록 최소한 Tunnel은 종료하세요.
