...
- 최종 릴리스 리허설, 장애 회고, 운영자 인수인계 문서를 포함해 PL Briefing과 Project Chat이 ready/watch/no-go 판단을 운영 관점으로 설명할 수 있습니다.
- release evidence 문서에는 한국어 판단 요약을 함께 넣어 한국어 Project Chat 질문이 영어 원문만 반복하지 않고 `준비 완료`, `주의`, `출시 불가`, `운영 인수인계`, `결제 대시보드 정합성 점검` 같은 표현을 근거로 사용할 수 있습니다.
...
- local LLM이 4096 token context로 실행되는 환경에서는 Project Chat demo 질문의 `TOP K`를 낮추거나 commit history를 제외해야 합니다. release/handoff 문서를 묻는 질문은 source_file 근거만으로도 충분히 답할 수 있게 설계합니다.
...
마지막 release evidence commit은 단순 완료 선언이 아니라 다음 자료를 함께 포함해야 합니다.

- `docs/release-evidence/release-rehearsal-2026-06-14.md`: 결제, 재고, 대시보드, 매출 리포트의 ready/watch/not-ready 상태
- `docs/release-evidence/go-no-go-summary.md`: PL Briefing이 ready/no-go를 분리해 요약할 수 있는 기준
- `docs/incidents/payment-dashboard-reconciliation-postmortem.md`: 대시보드 과집계와 결제/리포트 정합성 문제의 원인, 수정, 예방책
- `docs/operations/operator-handoff-checklist.md`: 운영자가 릴리스 후 매일 확인할 항목과 escalation 기준
...
- 릴리스 리허설에서 ready, watch, no-go로 나뉜 항목은 무엇인가요?
- 운영 인수인계에서 결제와 대시보드 정합성을 어떻게 확인하나요?
...
- review target 문서는 LLM 답변을 고정하는 정답지가 아니라, 사람이 봐도 왜 이 commit이 리뷰 대상인지 알 수 있는 업무 배경과 검토 기대치를 제공해야 합니다.
