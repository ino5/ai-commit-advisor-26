...
2026-06-14 결정에서는 중앙 서버 사용자가 개발자 PC의 미커밋 변경을 볼 수 없다는 점을 명확히 하면서도, 로컬 실행자와 운영자의 임시 점검 가능성을 위해 서버 working tree/staged 리뷰를 보조 옵션으로 남겼습니다. 이후 공개 Git URL 기반 관리형 clone/fetch가 도입됐고, 분석용 clone은 local 변경을 남기지 않은 채 remote branch와 일치시키는 운영 원칙이 더 분명해졌습니다. 서버 local 변경은 갱신을 막는 예외 상태인데 AI Code Review 선택지로 계속 제공하면 정상 workflow처럼 보입니다.

### 결정

- AI Code Review 실행 대상은 앱 서버 Git 저장소의 `latest_commit`과 사용자가 목록 또는 rev로 고른 `commit`만 지원합니다.
- UI와 `run_local_ai_verification.py`에서 `working_tree`와 `staged` 선택지를 제거하고, service도 해당 target 요청을 거부합니다.
- 기존 `code_review_results` row는 삭제하거나 변환하지 않습니다. 과거 `working_tree`와 `staged` 결과는 리뷰 기록에서 `(과거 기록)` label로 구분해 계속 읽을 수 있습니다.
- `target_type` column 구조는 그대로 두므로 schema migration은 만들지 않습니다.

### 이유

- Commit hash와 commit diff는 입력을 다시 조회할 수 있어 저장 리뷰의 근거와 실행 시점을 재현하기 쉽습니다.
- 중앙 앱 서버는 개발자 개인 PC의 working tree나 index를 읽지 못하므로 서버 local target은 개발자가 기대하는 commit 전 리뷰를 제공하지 않습니다.
- 관리형 clone의 local 변경은 fetch/reset을 방해하므로 리뷰 대상으로 활용하기보다 Git 상태 화면에서 발견하고 정리해야 합니다.
- 화면에서만 숨기지 않고 service와 검증 CLI까지 같은 지원 범위로 맞추면 우회 실행과 문서 불일치를 줄일 수 있습니다.

### 검토한 대안

- UI에서만 숨기고 service와 CLI 유지: 과거 호출 호환성은 넓지만 지원하지 않는 실행 경로가 남아 운영 검증과 제품 설명이 다시 갈릴 수 있어 제외했습니다.
- 고급 옵션 expander 안에 유지: 기본 화면은 줄어들지만 서버 clone local 변경을 정상 입력으로 취급하는 운영 충돌은 남습니다.
- 개발자 diff upload, Git hook, CI/webhook 연동으로 commit 전 리뷰 제공: 실제 개발자 변경을 받을 수 있지만 입력 신뢰 경계, 인증, 저장 정책과 실패 처리가 필요한 별도 기능이므로 이번 제거 범위에 포함하지 않았습니다.

### 영향, tradeoff, 남은 한계

- 사용자는 리뷰할 변경을 commit으로 만든 뒤 서버 저장소에 반영해야 합니다. 로컬 단독 실행에서도 미커밋 diff를 바로 리뷰하던 기능은 더 이상 제공하지 않습니다.
- 과거 결과를 보존하므로 저장 이력과 운영 증거는 유실되지 않지만, 해당 target으로 재실행할 수는 없습니다.
- Commit 전 리뷰가 필요해지면 서버 clone 상태를 다시 노출하기보다 CI/webhook이나 명시적인 diff 입력처럼 개발자 변경을 전달하는 경로를 별도 설계해야 합니다.

### 관련 문서

- `ROADMAP.md`의 `Remove Server-Local AI Code Review Targets`
- `docs/feature-guide.md`
- `docs/architecture.md`
- `docs/ai-technical-overview.md`
- `docs/demo-user-guide.md`
- `AI_CHANGELOG.md`의 `AI Code Review 서버 local 대상 제거`
...
Sample Shop의 개발자, 프로그램, 개발계획, 표준용어 Excel은 샘플 프로젝트의 `advisor_uploads` 폴더에 생성됩니다. 로컬 경로를 아는 사용자는 파일을 찾을 수 있지만, Docker나 공개 Git URL로 샘플을 등록한 사용자는 서버 내부 경로를 알아야 해 산출물 업로드 흐름을 바로 시작하기 어렵습니다.

### 결정

- 개발자 목록, 프로그램 목록, 개발계획, 표준용어/표준단어 화면의 `Excel 업로드` 탭에서 해당 Sample Shop Excel을 내려받을 수 있게 합니다.
- 샘플 버튼은 DB에 데이터를 직접 저장하지 않습니다. 사용자가 파일을 내려받아 기존 file uploader, 미리보기, validation, 저장 절차를 그대로 거칩니다.
- 샘플 Excel은 앱에서 생성해 로컬 샘플 저장소 경로와 Docker mount 여부에 의존하지 않습니다. 파일 column, row 수, 개발계획의 지연·미배정 시나리오가 현재 upload validator를 통과하는지 회귀 test로 확인합니다.
- 개발계획은 program ID와 developer ID를 참조하므로 같은 Sample Shop의 개발자와 프로그램을 먼저 저장해야 한다는 안내를 화면에 표시합니다.

### 이유

- 화면 안에서 샘플 파일을 찾을 수 있어 처음 사용하는 사람이 별도 경로 안내를 복사할 필요가 없습니다.
- 다운로드와 업로드를 분리하면 실제 Excel 검증 UX를 그대로 체험할 수 있고, 현재 프로젝트에 샘플 데이터가 자동으로 들어가는 것을 막을 수 있습니다.
- 기존 validator와 저장 서비스를 우회하지 않으므로 샘플 전용 저장 로직과 운영 데이터용 저장 로직이 갈라지지 않습니다.

### 검토한 대안

- `샘플 데이터 바로 등록` 버튼: 단계는 짧지만 현재 프로젝트를 즉시 변경하고, 기존 데이터와의 중복·갱신 범위를 사용자가 확인하기 어렵습니다.
- 샘플 프로젝트의 `advisor_uploads` 경로만 안내: 구현은 단순하지만 원격 clone, Docker, 사내 서버에서 파일 위치가 달라지는 문제가 남습니다.
- 빈 Excel 양식만 제공: schema 확인에는 충분하지만 Mapping, Risk Analysis, Project Chat까지 이어지는 Sample Shop 시나리오를 재현할 수 없습니다.

### 영향, tradeoff, 남은 한계

- 사용자는 다운로드한 파일을 다시 선택해야 하므로 한 번의 추가 동작이 필요합니다. 대신 저장 전 검증 결과를 확인할 수 있습니다.
- Sample Shop의 산출물 정의가 바뀌면 화면 다운로드 dataset도 함께 갱신해야 합니다. `tests/test_sample_artifact_service.py`가 현재 validator와의 불일치를 조기에 확인합니다.
- 이 기능은 Sample Shop 시연 데이터용입니다. 실제 프로젝트에 샘플을 저장하면 분석 결과도 샘플 기준으로 바뀌므로 현재 프로젝트를 확인한 뒤 저장해야 합니다.

### 관련 문서

- [샘플 프로젝트 처음 시작 가이드](sample-project-first-run-guide.md)
...
> 이 결정에서 보조 옵션으로 유지한 working tree/staged 실행 경로는 2026-07-23 `AI Code Review 실행 대상은 commit 이력으로 한정한다` 결정으로 대체했습니다. 과거 저장 결과 표시는 유지합니다.
