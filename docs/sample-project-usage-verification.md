...
| `PGVECTOR_DIMENSION` | `768` |
| `NEO4J_ENABLED` | `true` |
| 질문 | `PaymentController, PaymentService, OrderStatusService는 결제 승인 요청부터 주문 상태를 PAID로 바꾸는 흐름에서 어떻게 이어지는지 한국어로 설명해줘.` |
| 저장 session | `359` |
...
| used source 수 | 11 |
| graph evidence 수 | 8 |
| 핵심 graph evidence | `class_import PaymentService -> OrderStatusService` |
| 핵심 답변 근거 | `PaymentController.java`, `PaymentService.java`, `OrderStatusService.java`, `markPaid`, `PAID` |

이 검증의 통과 기준은 답변 문장 전체가 screenshot과 글자 단위로 같아지는 것이 아닙니다. 실제 provider가 verified `source_file`과 GraphRAG evidence를 사용해 `PaymentController`가 결제 승인 요청을 받고, `PaymentService`가 결제 저장과 감사 로그를 처리한 뒤 `OrderStatusService.markPaid(orderId)`로 주문 상태를 `PAID`로 바꾼다는 핵심 사실을 다시 설명해야 합니다.
...
.\.venv\Scripts\python.exe scripts\capture_feature_screenshot.py --url "http://localhost:8521/?project_id=97" --feature project-chat-graph-evidence --screenshot docs\images\usage-verification\project-chat-graph-repro-2026-06-17.png --surface local --height 1500 --expect-text "Provider: local_openai" --expect-text "fallback=False" --expect-text "PaymentService" --expect-text "OrderStatusService" --expect-text "class_import" --expect-text "markPaid" --forbid-text "Mock answer" --forbid-text "fallback=True" --forbid-text "StreamlitAPIException" --forbid-text "Traceback" --forbid-text "domain_summary"
```
