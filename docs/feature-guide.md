...
화면 폭이 768px 이하일 때 다른 메뉴를 선택하면 사이드바가 자동으로 닫혀 이동한 화면을 바로 보여줍니다. 데스크톱에서는 여러 화면을 이어서 탐색할 수 있도록 사이드바가 열린 상태를 유지합니다. 사이드바를 아래로 스크롤해도 상단 닫기 버튼은 고정되므로 메뉴 선택 없이 직접 닫을 수도 있습니다. 현재 메뉴를 다시 누르거나 메뉴 그룹만 펼치거나 현재 프로젝트를 바꾸는 동작은 자동 닫기 대상이 아닙니다.
