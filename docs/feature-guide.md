...
화면 폭이 768px 이하일 때 다른 메뉴를 선택하면 사이드바가 자동으로 닫혀 이동한 화면을 바로 보여줍니다. 데스크톱에서는 여러 화면을 이어서 탐색할 수 있도록 사이드바가 열린 상태를 유지합니다. 사이드바를 아래로 스크롤해도 상단 닫기 버튼은 고정되므로 메뉴 선택 없이 직접 닫을 수도 있습니다. 태블릿 터치 화면에서는 닫기 버튼을 항상 표시하고, 상단 고정 영역은 밝은 테마와 어두운 테마 모두 사이드바 배경색을 그대로 따릅니다. 현재 메뉴를 다시 누르거나 메뉴 그룹만 펼치거나 현재 프로젝트를 바꾸는 동작은 자동 닫기 대상이 아닙니다.

현재 프로젝트 선택은 Streamlit session state에만 저장되며 URL의 `project_id`를 읽거나 쓰지 않습니다. URL과 화면 선택값이 충돌하지 않으므로 사이드바에서 한 번 고른 프로젝트가 즉시 현재 작업 대상으로 반영됩니다. 새 브라우저 세션에서는 첫 프로젝트로 초기화될 수 있으며, 선택한 프로젝트가 삭제되면 남아 있는 첫 프로젝트로 복구합니다.
...
| 분석 실행 | AI Code Review | 최신 commit 또는 최근 목록에서 선택한 commit diff를 LLM으로 리뷰할 때 |
...
1. 프로젝트/Git 설정 화면에서 `Git URL에서 가져오기` 또는 `서버에 이미 있는 저장소 사용`을 선택해 프로젝트를 등록합니다.
...
7. Mapping 화면에서 커밋 기준 분석을 먼저 실행합니다.
8. Risk Analysis에서 리스크를 탐지하고, Program Detail, Commit Impact, Knowledge Graph, AI Progress에서 분석 결과를 검토합니다.
9. RAG Search와 Project Chat에서 현재 소스 기반 근거 검색과 질의응답을 확인합니다.
10. AI Code Review에서 최신 commit을 바로 리뷰하거나 최근 목록에서 대상을 선택합니다.

## Git 저장소 기반 샘플 Excel 생성 CLI

일반적인 Sample Shop 확인에는 각 산출물 관리 화면의 `Excel 업로드` 탭에서 제공하는 샘플 파일을 사용합니다. 별도의 로컬 Git 저장소를 대상으로 업로드 형식이나 데이터 흐름을 검증해야 할 때만 `scripts/generate_sample_development_data.py`를 실행합니다.

```powershell
.\.venv\Scripts\python.exe scripts\generate_sample_development_data.py `
  --repo-path C:\dev\your-repository `
  --output-dir .\sample_data
```

명령이 정상적으로 끝나면 출력 폴더에 다음 파일이 생성됩니다.

- `sample_developers.xlsx`: commit author를 기준으로 만든 개발자 목록
- `sample_programs.xlsx`: 변경 파일의 module과 Controller, Service, Mapper/Repository 이름을 기준으로 만든 프로그램 목록
- `sample_development_plan.xlsx`: commit date와 프로그램별 변경 이력을 기준으로 만든 개발계획

저장소 안의 `*프로그램목록*.csv`를 우선 사용하려면 `--use-existing-program-csv`를 추가합니다. 특정 CSV를 지정할 때는 두 옵션을 함께 사용합니다.
...
.\.venv\Scripts\python.exe scripts\generate_sample_development_data.py `
  ...
  --program-csv-path C:\dev\your-repository\프로그램목록.csv
```

이 CLI는 Excel 파일만 만들며 프로젝트나 DB에 자동 저장하지 않습니다. 생성된 값은 Git 이력에서 추정한 가상 데이터이므로 실제 개발자 역할, 일정, 진척도로 취급하지 말고 업로드 검증이나 데모 준비에만 사용합니다. 앱에 반영하려면 각 산출물 관리 화면의 `Excel 업로드` 탭에서 파일을 선택하고 검증 결과를 확인한 뒤 저장합니다.

## 프로젝트 관리와 삭제

프로젝트/Git 설정 화면은 프로젝트 이름, 설명, Git 저장소 준비 방식을 관리합니다. 공개 HTTPS Git URL 방식은 서버가 관리형 경로를 자동 배정해 clone/fetch하고, 기존 경로 방식은 운영자가 준비한 서버 저장소를 그대로 분석합니다. 기존 프로젝트를 선택하면 마지막 Git 동기화 상태와 저장소 상태를 확인할 수 있고, 같은 화면에서 분석 데이터 초기화나 프로젝트 삭제를 실행할 수 있습니다.
...
산출물 관리 메뉴의 개발자 목록, 프로그램 목록, 개발계획 데이터는 Excel 업로드와 화면 기반 직접 관리를 모두 지원합니다. 개발자 목록, 프로그램 목록, 개발계획, 표준용어/표준단어의 `Excel 업로드` 탭에는 Sample Shop에서 사용한 Excel 다운로드가 함께 있어 별도 파일 경로 없이 업로드 검증 흐름을 확인할 수 있습니다. 샘플 파일은 자동 저장되지 않으며, 내려받은 파일을 업로더에서 선택하고 검증 결과를 확인한 뒤 저장합니다.
...
AI Progress는 계획 진척도와 프로그램 단위 구현상태 분석을 비교합니다. 저장된 분석이 현재 관련 commit 묶음과 일치할 때만 AI Progress 평균과 차이에 사용하며, 분석이 없거나 오래되었으면 `분석 필요` 또는 `재분석 필요`로 표시합니다. 기존 Mapping 기반 0/50/100 값은 관련 commit 근거를 빠르게 확인하기 위한 `Mapping 참고 진척도`로만 함께 보여줍니다.
...
동기화 후에는 클래스 관계도, 영향 경로, node/edge 저장 상태를 Neo4j에 저장된 그래프에서 다시 조회해 보여줍니다. `관계 탐색` 보기에서는 프로그램, 클래스, 도메인, 커밋 중 하나를 선택해 주변 path를 깊이와 관계 종류로 좁혀 볼 수 있습니다. 조회 결과는 선택 node를 파란 테두리로 표시한 관계도와 path 표에 함께 나타납니다. 화살표는 Neo4j에 저장된 관계 방향을 따르고, node 종류는 색과 모양으로 구분되며, edge에 마우스를 올리면 관계 이름을 확인할 수 있습니다. 화면이 복잡해지지 않도록 관계도는 최대 node 12개와 edge 16개만 그리지만, 조회된 path 전체는 아래 표에 유지됩니다. 기본 깊이는 2이고 필요할 때 1~3 범위에서 바꿀 수 있습니다. Neo4j 동기화 결과의 `Neo4j 동기화 실행 세부`에서는 batch 크기, 완료 batch 수, retry 수, 실패 operation을 확인할 수 있습니다. `동기화 준비 경고`에 generated source, build output, test fixture, type 선언 없음이 표시되면 해당 파일은 graph class/import 근거에서 제외됐다는 뜻입니다. Neo4j 없이 가볍게 실행해야 할 때만 `NEO4J_ENABLED=false`로 바꾸면 preview는 유지되고 저장 동기화와 저장 그래프 조회만 건너뜁니다.
...
- 선택한 프로그램, 클래스, 도메인, 커밋 주변의 방향성 관계도, path 표, node properties 확인
...
Risk Analysis는 프로그램 목록, 개발계획, Git 변경, 최신 프로그램 단위 구현상태를 비교해 먼저 점검할 위험 신호를 규칙으로 찾습니다. 이 결과는 장애나 일정 지연을 확정하는 판정이 아니라 프로그램 상세와 담당자 확인으로 이어지는 검토 목록입니다.
...
Project Chat 화면의 `대화 관리` 영역에서 프로젝트별 저장된 session을 선택할 수 있고, `새 대화 시작`은 기존 이력을 지우지 않고 새 session을 만듭니다. Assistant 답변에는 provider/model, fallback, `답변 근거 검증`, 실제 사용한 source/graph 수가 표시되고 `답변 근거 보기`와 `그래프 관계 근거 보기`가 분리되어 있습니다. Java 파일을 지정한 관계 질문은 현재 파일의 메서드 근거로 직접 호출 순서와 조건식을 검증합니다. local LLM 문장이 이 검증을 통과하지 못해도 근거가 충분하면 `deterministic_repair`로 안전한 답변을 재구성하며, repair 여부를 숨기지 않습니다. `그래프 관계 근거 보기`는 GraphRAG evidence를 compact node-edge 관계도와 표로 나누어 보여주며, 원본 metadata는 필요할 때만 체크박스로 펼칩니다. 이 관계도는 답변에 쓰인 주변 근거를 빠르게 확인하기 위한 view이고, 전체 graph 탐색은 `Knowledge Graph` 화면에서 수행합니다. `근거 복사용 Markdown` 영역은 답변, 현재 소스 근거, 이력/참고 근거, 그래프 관계 근거를 회의록이나 리뷰 문서에 붙여 넣기 쉬운 형식으로 제공합니다.
...
Project Chat에 들어가면 저장된 근거 수와 검색 준비 수, 현재 Repo HEAD와 indexed HEAD의 일치 여부를 먼저 표시합니다. 이 단계에서는 저장소 전체 파일을 읽지 않으므로 `HEAD 일치 · 파일 확인 전`으로 보일 수 있습니다. 전체 파일 내용이 저장된 hash와 같은지 확인하려면 `근거 상태 새로고침`을 누르세요. 파일 수와 Docker mount 환경에 따라 시간이 걸릴 수 있으며, 화면은 확인 중인 작업을 spinner로 안내합니다. 같은 project/HEAD/source index/embedding 설정에서는 이 결과를 현재 화면 session에서 재사용합니다.

현재 소스 파일을 수정하거나 브랜치/HEAD가 바뀐 뒤에는 `답변 근거 상태`를 확인하고 필요 시 `최신 변경분 반영` 또는 `전체 소스 다시 읽기`를 실행하세요. 질문을 보내면 검색된 근거 파일을 답변 전에 직접 검증하므로, 전체 파일 검증을 실행하지 않았다는 이유로 stale 근거를 현재 코드처럼 사용하지 않습니다. 화면의 `검색 준비`가 전체 근거 수보다 적으면 `RAG 검색 > 검색 준비`에서 남은 작업을 제한 수량으로 실행합니다. 메뉴 진입만으로 LLM, embedding 생성, 전체 source re-index, Knowledge Graph sync가 실행되지는 않습니다.

`현재 프로젝트`, `TOP K`, `검색 준비`, `근거 상태 새로고침`, `최신 변경분 반영`, `전체 소스 다시 읽기`처럼 의미를 바로 알기 어려운 control에는 물음표 도움말이 붙어 있습니다. 화면 설명을 길게 늘리지 않기 위해 기본 화면에는 짧은 label만 두고, 필요한 경우 물음표에 마우스를 올려 기준과 사용 시점을 확인합니다.

## AI Code Review

AI Code Review는 앱 서버 Git 저장소의 commit 이력을 LLM으로 리뷰합니다. 중앙 앱 서버에서 실행하는 검증에서는 개발자 개인 PC의 작업트리나 staged 변경을 직접 볼 수 없으므로, 기본 흐름은 `최신 커밋` 또는 `커밋 목록에서 선택` 리뷰입니다.

지원 대상:

- 최신 commit: `HEAD`
- 목록에서 선택한 commit: 현재 branch의 최근 50개 commit을 hash, 시각, 작성자, 메시지로 확인한 뒤 선택
- 목록에 없는 commit: commit hash, branch, tag 또는 `main~2` 같은 rev 직접 입력

개발자 개인 PC의 미커밋 변경과 앱 서버 clone의 working tree/staged 변경은 리뷰 대상으로 제공하지 않습니다. 리뷰할 변경은 commit으로 만든 뒤 서버 저장소에 반영하세요. 과거 버전에서 만든 서버 local 변경 리뷰는 삭제하지 않으며, 리뷰 기록에서 `(과거 기록)` 표시로 계속 확인할 수 있지만 다시 실행할 수는 없습니다.

리뷰 결과는 `code_review_results` 테이블에 저장되며, 변경 의도, 영향 범위, 위험도, 버그 후보, 리팩토링 제안을 포함합니다.

리뷰 설명이 영어 위주로 생성되면 앱은 finding 수, 파일·라인, severity, 위험도 같은 판단 구조를 그대로 유지한 채 사용자 설명 필드만 한국어로 고치는 호출을 한 번 시도합니다. 보정 결과가 한국어와 구조 보존 검증을 모두 통과하면 보정본을 저장합니다. 끝까지 영어이거나 보정 과정에서 구조가 바뀌면 영어 원문도 유효한 리뷰로 보고 화면과 리뷰 기록에 그대로 제공하며, 언어 검증 실패는 AI 호출 기록과 application log에만 남깁니다. 따라서 화면의 `완료`는 리뷰 생성과 저장이 끝났다는 뜻이며 모든 설명이 한국어라는 보장은 아닙니다.

## 대시보드

- 개발계획 대시보드: 프로그램 목록, 상태별 프로그램 수, 개발자별 배정 프로그램 수, 지연 프로그램, 계획 대비 완료율
- Home: 현재 프로젝트 KPI와 함께 프로젝트/Git/프로그램/Mapping/소스 근거/검색 준비/Knowledge Graph 상태별 다음 준비 작업을 보여줍니다.
...
- 운영 준비: DB, Git, 산출물, Mapping, AI Progress, Risk Analysis, LLM/embedding 설정, source index, PL Briefing, AI 호출 기록 상태를 `통과/주의/실패`로 보여줍니다. 상단의 `AI 실행 바로가기`에서 Mapping, Risk Analysis, PL Briefing, 검색 준비를 바로 실행할 수 있고, `다음 준비 작업`은 비어 있는 프로젝트/Git/프로그램/Mapping/source/vector/Knowledge Graph 상태를 어떤 화면에서 채워야 하는지 먼저 안내합니다.
