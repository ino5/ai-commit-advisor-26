...
화면은 현재 프로젝트 기준 그래프 preview를 먼저 구성한 뒤, 기본 설정에서는 `Neo4j 동기화`로 graph read model을 저장합니다. 동기화 후에는 클래스 관계도, 영향 경로, node/edge 저장 상태를 Neo4j에 저장된 그래프에서 다시 조회해 보여줍니다. Neo4j 없이 가볍게 실행해야 할 때만 `NEO4J_ENABLED=false`로 바꾸면 preview는 유지되고 저장 동기화와 저장 그래프 조회만 건너뜁니다.
