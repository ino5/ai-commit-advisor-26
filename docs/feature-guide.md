...
Project Chat은 현재 파일 내용과 일치하는 소스 chunk만 기본 근거로 사용합니다. 커밋 diff는 변경 이력으로 취급하며 현재 코드 근거와 구분합니다.

## 권장 사용 순서

1. Project 화면에서 로컬 Git 저장소 경로를 등록합니다.
2. Git 화면에서 전체 수집 또는 증분 동기화로 commit과 diff를 저장합니다.
3. Developer 화면에서 Git author 기반 개발자 자동 추출 또는 Excel 업로드를 수행합니다.
4. 프로그램 관리 화면에서 프로그램 목록을 직접 관리하거나 Excel로 업로드합니다.
5. 개발계획 관리 화면에서 담당자, 일정, 상태, 진행률을 등록합니다.
6. Mapping 화면에서 커밋 기준 분석을 먼저 실행합니다.
7. Program Detail, Commit Impact, Risk Analysis, AI Progress에서 분석 결과를 검토합니다.
8. RAG Search와 Project Chat에서 현재 소스 기반 근거 검색과 질의응답을 확인합니다.
9. AI Code Review에서 작업트리, staged 변경, 최신 commit, 특정 commit을 리뷰합니다.

## Data Management

개발자, 프로그램, 개발계획 데이터는 Excel 업로드와 화면 기반 직접 관리를 모두 지원합니다.
...
## Program Detail And AI Progress
...
## RAG Search And Project Chat

RAG Search는 현재 소스 파일, 프로그램 정보, commit 메시지, 변경 파일/diff를 chunk로 만들고 embedding을 저장해 검색합니다.

Project Chat은 현재 파일과 일치한다고 검증된 `source_file` chunk만 기본 답변 근거로 사용합니다. 검증된 현재 소스 근거가 없으면 추측성 답변을 만들지 않고 추가 인덱싱 또는 검색어 조정이 필요하다고 안내합니다.

현재 소스 파일을 수정하거나 브랜치/HEAD가 바뀐 뒤에는 RAG 또는 Project Chat 화면에서 인덱스 상태를 확인하고 필요 시 현재 소스를 다시 인덱싱하세요.

## AI Code Review

AI Code Review는 로컬 Git 저장소의 변경 사항 또는 commit을 LLM으로 리뷰합니다.
