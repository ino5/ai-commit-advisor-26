...
`nomic-embed-text-v2-moe`는 질의와 문서의 task prefix가 서로 다릅니다. 애플리케이션이 이 prefix를 자동으로 붙이므로 LM Studio에는 원문을 그대로 전달하도록 별도 변환을 설정하지 않습니다.
...
운영 model을 바꾸기 전에는 같은 LM Studio endpoint에서 후보를 하나씩 로드하고 아래 스크립트를 실행합니다.
...
두 실행은 같은 3개 구조화 문제와 같은 JSON Schema를 사용합니다. `valid_json_count`, `expected_result_count`, `total_seconds`, 각 case의 `reasoning_tokens`를 비교하고, `lms ps --json`과 `nvidia-smi`로 embedding 동시 로드 상태의 VRAM 여유도 확인합니다. Qwen3 계열은 애플리케이션이 `reasoning_effort=none`을 전달하므로 `reasoning_tokens=0`이어야 합니다.

이 비교는 후보를 빠르게 거르는 진단입니다. 실제 기본 model 전환은 Mapping, Project Chat, AI Code Review, PL Briefing의 실제 프로젝트 결과와 fallback 비율까지 별도로 확인한 뒤 결정합니다.
