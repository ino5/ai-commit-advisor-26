...
샘플 프로젝트는 8개 program과 30개 commit으로 Git Sync, Mapping, Program Detail, Commit Impact, Risk Analysis, RAG, Project Chat, AI Code Review, AI Progress를 보여주도록 설계되어 있습니다.
...
반복 가능한 screenshot verification을 위해, 이전 샘플 데이터와 섞이지 않도록 전용 project name을 사용합니다.
...
전체 demo scenario에는 새로 추론한 일반 output이 아니라 아래 file을 사용합니다.
...
`advisor_uploads` file에는 지연된 coupon 작업, settlement 담당자 누락, 관련 commit 없음 risk scenario를 보여주기 위한 데모 전용 plan override가 포함되어 있습니다.
...
샘플 프로젝트는 Spring MVC + JSP + MyBatis입니다. Project Chat verification 전 current source indexing 대상에 Java, JSP, XML, Markdown, JavaScript, CSS file이 포함되어야 합니다.
...
README screenshot을 갱신할 때는 위에서 검증한 샘플 상태를 사용합니다. Screenshot은 다음을 반영해야 합니다.

- 8개 sample programs
- 30개 sample commits
- commit-based analysis에서 나온 Mapping result
- coupon과 settlement 확인 포인트가 보이는 Risk Analysis
...
- 프로젝트가 `C:\dev\ai-advisor-sample-shop`을 사용하는지 확인합니다.
