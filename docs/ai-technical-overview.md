...
| AI Evidence trace/telemetry | AI Evidence | 저장된 AI 결과의 provider/model, fallback, validation, source evidence, raw metadata, 호출 latency를 추적합니다. | PoC에서 AI가 실제로 어떤 근거와 운영 상태로 동작했는지 설명할 수 있게 합니다. |
...
| AI Evidence | 저장된 AI 결과와 호출 telemetry를 추적하고 PoC 준비 상태를 점검 | PL Briefing history, Mapping raw response, Project Chat sources, Code Review result, AI invocation logs | Readiness, evidence trace, scorecard, weekly report, telemetry |
...
`AI Evidence` 화면은 이 추적 데이터를 시연용으로 묶습니다. Readiness cockpit은 DB, Git, 산출물, LLM/embedding 설정, source index, PL Briefing, telemetry 상태를 점검합니다. Evidence trace는 PL Briefing, Mapping, Project Chat, AI Code Review의 저장 근거를 읽기 전용으로 보여주며, scorecard는 sample project 기준으로 주요 AI 결과가 시연 가능한 최소 신호를 갖췄는지 확인합니다. 주간 보고서 export는 Radar, 최신 PL Briefing, 미해결 리스크, AI Progress gap, telemetry 요약을 Markdown으로 묶습니다.
...
- AI Evidence scorecard는 sample project와 현재 저장 데이터 기준의 점검표이며, 모델 품질을 통계적으로 평가하는 benchmark는 아닙니다.
