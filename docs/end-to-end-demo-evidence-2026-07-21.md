...
> 이 절은 2026-07-21 검증 당시의 관찰 기록입니다. commit 소유권 이동 제한은 2026-07-22 revision `20260722_0011`과 project-scoped Git Sync로 해소됐으며, 현재 시연 기준 프로젝트를 하나로 유지하는 운영 원칙과는 별개입니다.

- 검증 당시 `git_commits.commit_hash`에는 전역 unique 제약이 남아 있어 같은 저장소의 중복 프로젝트를 유지하지 않고 project `2716` 하나만 기준으로 사용했습니다.
