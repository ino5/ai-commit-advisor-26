...
## Q. 프론트엔드·백엔드·DB는 어떤 기술 스택이며 Streamlit과 Python service는 어떻게 연결되나?

이 프로젝트에는 React 같은 별도 프론트엔드 앱이나 FastAPI·Flask·Django 기반 REST API 서버가 없습니다. `Streamlit 1.41.1`이 브라우저 화면을 구성하는 UI framework이면서, Python 코드를 실행하고 브라우저 연결과 사용자 session을 처리하는 유일한 web server runtime입니다. 브라우저에는 Streamlit이 제공하는 frontend client가 로드되지만, 이 repository에서 별도의 JavaScript·TypeScript frontend project를 개발하거나 배포하지는 않습니다.

`Python 3.11 service layer`는 별도 백엔드 framework 이름이 아닙니다. `src/services/`와 `src/rag/`에 분리한 일반 Python 함수·class 모듈을 뜻합니다. Streamlit page가 이 모듈을 import해 같은 앱 process 안에서 직접 호출하므로 UI와 service 사이에 REST API나 JSON 통신이 없습니다.
...
Streamlit server (`streamlit run app.py`)
  ...
  → widget 클릭 시 page Python 코드 rerun
...
따라서 “백엔드도 Streamlit인가?”라는 질문에는 실행 경계와 코드 역할을 나눠 답해야 합니다.

- Web server와 사용자 요청·session 처리 관점에서는 맞습니다. 별도 API server 없이 Streamlit server가 앱의 진입점입니다.
- 업무 로직 관점에서는 Streamlit widget 코드와 분리된 일반 Python service layer가 백엔드 역할을 합니다. 다만 별도 process나 network service가 아니라 Streamlit app process 내부 모듈입니다.
...
| 영역 | 주요 기술 | 역할 |
|---|---|---|
| Web UI·server runtime | `Streamlit 1.41.1` | 브라우저 화면, widget event, session state, Python page rerun과 HTTP server 제공 |
| UI page 계층 | `app.py`, `src/ui/*.py` | 메뉴·form·table 구성, 입력 검증과 service 호출 조정 |
| 업무 service 계층 | `Python 3.11`, `src/services/*.py`, `src/rag/*.py` | Git Sync, Mapping, Risk Analysis, RAG, Project Chat, Code Review와 Graph 동기화 처리. 별도 web framework 없음 |
