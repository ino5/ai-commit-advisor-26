...
- 프로그램 관리: 현재 데이터 검색, Excel 양식 다운로드, 업로드 전 검증/미리보기, 신규/수정 요약
- 개발계획 Excel 업로드
...
- 사용 중: 검색 결과 source_file chunk가 현재 파일과 일치하는지 verified/stale/invalid로 검증 (`src/rag/source_verifier.py`)
...
현재 소스 파일 chunk에는 `file_path`, `line_start`, `line_end`, `content_hash`, `chunk_content_hash`, `indexed_head_hash` 메타데이터가 저장됩니다. Search 탭은 검색어, 조회된 chunk 목록, cosine 유사도 점수, chunk 출처(source_file/program/commit/commit_file), 검증 상태, 원문 일부를 함께 표시합니다.

Project Chat은 검증된 `source_file` chunk만 기본 답변 근거로 사용합니다. 인덱싱 후 파일이 수정되었거나 HEAD가 바뀐 chunk는 stale/invalid로 분류하고 답변 근거에서 제외합니다. 커밋 이력은 옵션으로 검색 후보에 포함할 수 있지만, 현재 코드 근거로는 사용하지 않습니다.
...
프로그램 관리 화면에서 현재 등록된 프로그램을 조회하고, Excel 양식을 내려받고, 업로드 전 검증 후 저장할 수 있습니다.
