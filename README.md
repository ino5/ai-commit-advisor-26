...
Project Chat은 현재 파일 내용과 일치하는 소스 chunk만 기본 근거로 사용합니다. 커밋 diff는 변경 이력으로 취급하며 현재 코드 근거와 구분합니다. RAG와 Project Chat 화면에서는 현재 Git HEAD와 인덱싱 시점 HEAD를 비교해 재인덱싱 필요 여부를 보여주고, 버튼 한 번으로 현재 소스 인덱스를 다시 만들 수 있습니다.
...
- 현재 소스 인덱스 상태 확인: Current HEAD, Indexed HEAD, 불일치/검증 불가 chunk 수 표시와 원클릭 재인덱싱
...
RAG와 Project Chat 화면의 `현재 소스 다시 인덱싱` 버튼은 기존 `source_file` chunk/vector를 정리한 뒤 현재 Git HEAD 기준으로 다시 chunk와 embedding을 생성합니다. 파일 삭제처럼 기존 증분 인덱싱으로 남을 수 있는 오래된 근거도 이 경로에서 제거됩니다.
