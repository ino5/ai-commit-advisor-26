...
모바일에서는 사이드바 메뉴로 다른 화면을 선택하면 메뉴가 자동으로 닫힙니다. 데스크톱에서는 연속 탐색을 위해 열린 상태를 유지하며, 화면 크기와 관계없이 사이드바를 아래로 스크롤해도 상단 닫기 버튼은 고정됩니다. 태블릿 터치 화면에서도 닫기 버튼을 바로 사용할 수 있고, 상단 고정 영역은 현재 테마와 같은 배경으로 표시됩니다.
...
임의의 로컬 Git 저장소에서 업로드 검증용 개발자·프로그램·개발계획 Excel을 만들 때는 별도 CLI를 사용합니다. 이 명령은 DB를 변경하지 않고 지정한 출력 폴더에 파일만 생성합니다.
...
생성값은 commit author, commit date, 변경 파일을 바탕으로 만든 가상 데이터이므로 실제 인력 배치나 개발계획으로 사용하지 않습니다. 기존 프로그램목록 CSV를 우선 사용하는 옵션과 생성 파일 설명은 [기능 가이드의 CLI 안내](docs/feature-guide.md#git-저장소-기반-샘플-excel-생성-cli)를 참고하세요.
...
- Neo4j Knowledge Graph에서 프로젝트, 프로그램, 커밋, 파일, 클래스, 도메인 관계를 저장하고 Graph HEAD 최신성, 선택 node 주변 관계도와 path 표, 영향 경로를 확인합니다. 선택 node는 파란 테두리로 강조되고 화살표는 저장된 관계 방향을 따릅니다. Java parser는 annotation type, static import, nested member type을 반영하고 generated/build/test fixture 제외 경고를 표시합니다.
